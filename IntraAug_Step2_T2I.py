...
import numpy as np
import abc
import utils.ptp_utils
import utils.seq_aligner
import os
import cv2
import argparse
...
ldm_stable = StableDiffusionPipeline.from_pretrained("pretrained/roentgen").to(device)
    ...
    def __init__(self, prompts: List[str], words: [List[List[str]]], threshold=.3):
        ...
        for i, (prompt, words_) in enumerate(zip(prompts, words)):
            if type(words_) is str:
                words_ = [words_]
            for word in words_:
                ind = utils.ptp_utils.get_word_inds(prompt, word, tokenizer)
...
class AttentionControlEdit(AttentionStore, abc.ABC):
    ...
    def __init__(self, prompts, num_steps: int,
        ...
        self.cross_replace_alpha = utils.ptp_utils.get_time_words_attention_alpha(prompts, num_steps, cross_replace_steps,
                                                                            tokenizer).to(device)
...
class AttentionReplace(AttentionControlEdit):
    ...
    def __init__(self, prompts, num_steps: int, cross_replace_steps: float, self_replace_steps: float,
        ...
        self.mapper = utils.seq_aligner.get_replacement_mapper(prompts, tokenizer).to(device)
...
class AttentionRefine(AttentionControlEdit):
    ...
    def __init__(self, prompts, num_steps: int, cross_replace_steps: float, self_replace_steps: float,
        ...
        self.mapper, alphas = utils.seq_aligner.get_refinement_mapper(prompts, tokenizer)
...
def get_equalizer(text: str, word_select: Union[int, Tuple[int, ...]], values: Union[List[float],
    ...
    for word in word_select:
        inds = utils.ptp_utils.get_word_inds(text, word, tokenizer)
...
def show_cross_attention(attention_store: AttentionStore, res: int, from_where: List[str], select: int = 0):
    ...
    for i in range(len(tokens)):
        ...
        image = utils.ptp_utils.text_under_image(image, decoder(int(tokens[i])))
        images.append(image)
    utils.ptp_utils.view_images(np.stack(images, axis=0))
...
def show_self_attention_comp(attention_store: AttentionStore, res: int, from_where: List[str],
    ...
    for i in range(max_com):
        ...
        images.append(image)
    utils.ptp_utils.view_images(np.concatenate(images, axis=1))
...
def run_and_display(prompts, controller, latent=None, run_baseline=False, generator=None):
    ...
    images, x_t = utils.ptp_utils.text2image_ldm_stable(ldm_stable, prompts, controller, latent=latent,
                                                  num_inference_steps=NUM_DIFFUSION_STEPS,
                                                  guidance_scale=GUIDANCE_SCALE, generator=generator,
                                                  low_resource=LOW_RESOURCE)
    utils.ptp_utils.view_images(images)
...
for _, _, files in os.walk(text_path_gpt):
    final_files = sorted(files)

save_dir = 'data/IntraAug/IntraAug_images/'
